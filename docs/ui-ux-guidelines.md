...
- In the order builder, sugar is chosen per cup via a `درجة السكر` dropdown (Select) — options سادة / ريحة / مزبوط / مانو / زيادة / فوق الزيادة / كراميل — shown beside the per-line note only for products whose **category** advertises sugar capability; products from other categories show the note alone and never expose the sugar picker. Changing a multi-cup line's sugar visibly splits one cup off. Payment mirrors the POS: method toggles with amounts, cash tendered + change, and a clear "المدفوعات يجب أن تساوي الإجمالي" hint until the payment matches.
